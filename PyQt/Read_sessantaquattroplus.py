        ...
        while self.running:
            try:
                ...
                for track in self.tracks:
                    track.feed(
                        reshaped_data[
                            channel_index : channel_index + track.num_channels, :
                        ]
                    )
...
class Soundtrack(QtWidgets.QWidget):
    def __init__(self, device, client_socket):
        ...
        self.receiver_thread = DataReceiverThread(
            self.device, self.client_socket, self.tracks
        )
    ...
    def init_tracks(self):
        if self.device.nchannels == 72:  # Full configuration
            ...
            track_info = [
                (
                    f"HDsEMG {main_channels} channels",
                    main_channels,
                    0,
                    0.001,
                    0.000000286,
...
class SessantaquattroPlus:
    ...
    def create_command(
        self, FSAMP=2, NCH=3, MODE=0, HRES=0, HPF=0, EXTEN=0, TRIG=0, REC=0, GO=1
    ):
